...
public:
	...
	// 返回值future<>   引用折叠(与模版使用)
	template<typename Func,typename... Args>
	auto submitTask(Func&& func, Args&&... args) -> std::future<decltype(func(args...))> // 推导出函数返回值类型
	{
		// 打包任务，放入任务队列里面
		using RType = decltype(func(args...)); // decltype可以推导出func(args...)的返回值类型
		auto task = std::make_shared<std::packaged_task<RType()>>( // std::packaged_task<RType()>是指针指向的对象的类型
			std::bind(std::forward<Func>(func), std::forward<Args>(args)...)); // 给packaged_task赋值
		std::future<RType> result = task->get_future(); // 获取返回结果
			...
			[&]()->bool {return taskQue_.size() < (size_t)taskQueMaxThreshHold_; })) // 如果满足taskQue_.size() < taskQueMaxThreshHold_就说明可以继续执行
		{
			...
			auto task = std::make_shared<std::packaged_task<RType()>>(
				[]()->RType {return RType(); });  // 提交失败返回一个RType零值
			(*task)()
		...
		// 由于任务队列的每个函数对象都是void返回值，我们自定义一个没有返回值的函数对象作为中间层来执行任务
		taskSize_++;
	...
	// 开启线程池
	void start(int initThreadSize = std::thread::hardware_concurrency()) // hardware_concurrency 获取CPU的核心数量
	{
		// 设置线程池的启动状态
		isPoolRunning_ = true;
...
private:
	...
	size_t init_threadSize_; // 初始的线程数量
	int threadSizeThreshHold_; // 线程数量上限阈值
	std::atomic_int  curThreadSize_; // 记录当前线程池里面线程的总数量
	std::atomic_int idleThreadSize_; // 记录空闲线程的数量

	// Task任务 =》函数对象
	using Task = std::function<void()>; // 由于这里不知道传进来的任务返回值是什么，先用void，上面增加一个中间层来执行任务
	std::queue<Task> taskQue_; // 任务队列 
	std::atomic_int taskSize_; // 任务的数量 线程安全
	int taskQueMaxThreshHold_; // 任务队列数量上线阈值

	std::mutex taskQueMtx_; // 保证任务队列的线程安全 锁
